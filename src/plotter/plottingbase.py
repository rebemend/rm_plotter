...
class Plottable(object):
    ...
    def _clone_post_init(self, obj=None, **kwargs):  # noqa: C901
        ...
        if obj is None or obj is self:
            ...
            if isinstance(self.th, ROOT.TAttLine):
                self._linecolor = Color(ROOT.TAttLine.GetLineColor(self.th))
                self._linestyle = LineStyle(ROOT.TAttLine.GetLineStyle(self.th))
                self._linewidth = ROOT.TAttLine.GetLineWidth(self.th)
            else:  # HistStack
            ...
            if isinstance(self.th, ROOT.TAttFill):
                self._fillcolor = Color(ROOT.TAttFill.GetFillColor(self.th))
                self._fillstyle = FillStyle(ROOT.TAttFill.GetFillStyle(self.th))
            else:  # HistStack
            ...
            if isinstance(self.th, ROOT.TAttMarker):
                self._markercolor = Color(ROOT.TAttMarker.GetMarkerColor(self.th))
                self._markerstyle = MarkerStyle(ROOT.TAttMarker.GetMarkerStyle(self.th))
                self._markersize = ROOT.TAttMarker.GetMarkerSize(self.th)
            else:  # HistStack
            ...
            if isinstance(obj.th, ROOT.TAttLine):
                self.SetLineColor(obj.th.GetLineColor())
                self.SetLineStyle(obj.th.GetLineStyle())
                self.SetLineWidth(obj.th.GetLineWidth())
            if isinstance(obj.th, ROOT.TAttFill):
                self.SetFillColor(obj.th.GetFillColor())
                self.SetFillStyle(obj.th.GetFillStyle())
            if isinstance(obj.th, ROOT.TAttMarker):
                self.SetMarkerColor(obj.th.GetMarkerColor())
                self.SetMarkerStyle(obj.th.GetMarkerStyle())
    ...
    def decorate(self, other=None, **kwargs):  # noqa: C901
            ...
            if incompatible:
                raise ValueError(
                    ...
                    "is ambiguous. Please set only one.".format(
                        ", ".join(incompatible), "s" if len(incompatible) != 1 else ""
                    )
                )
        ...
        for key, value in kwargs.items():
            if key in Plottable.EXTRA_ATTRS:
                ...
                raise AttributeError("unknown decoration attribute: `{0}`".format(key))
        return self
    ...
    def SetLineColor(self, color):
        ...
        *color* may be any color understood by ROOT or matplotlib.
        ...
        if isinstance(self.th, ROOT.TAttLine):
            ROOT.TAttLine.SetLineColor(self.th, self._linecolor("root"))
    ...
    def SetLineStyle(self, style):
        ...
        *style* may be any line style understood by ROOT or matplotlib.
        ...
        if isinstance(self.th, ROOT.TAttLine):
            ROOT.TAttLine.SetLineStyle(self.th, self._linestyle("root"))
    ...
    def SetFillColor(self, color):
        ...
        *color* may be any color understood by ROOT or matplotlib.
        ...
        if isinstance(self.th, ROOT.TAttFill):
            ROOT.TAttFill.SetFillColor(self.th, self._fillcolor("root"))
    ...
    def SetFillStyle(self, style):
        ...
        *style* may be any fill style understood by ROOT or matplotlib.
        ...
        if isinstance(self.th, ROOT.TAttFill):
            ROOT.TAttFill.SetFillStyle(self.th, self._fillstyle("root"))
    ...
    def SetMarkerColor(self, color):
        ...
        *color* may be any color understood by ROOT or matplotlib.
        ...
        if isinstance(self.th, ROOT.TAttMarker):
            ROOT.TAttMarker.SetMarkerColor(self.th, self._markercolor("root"))
    ...
    def SetMarkerStyle(self, style):
        ...
        *style* may be any marker style understood by ROOT or matplotlib.
        ...
        if isinstance(self.th, ROOT.TAttMarker):
            ROOT.TAttMarker.SetMarkerStyle(self.th, self._markerstyle("root"))
...
def convert_markerstyle(inputstyle, mode, inputmode=None):  # noqa:C901
    ...
    if inputmode is None:
        if inputstyle in markerstyles_root2mpl:
            ...
            raise ValueError("`{0}` is not a valid `markerstyle`".format(inputstyle))
    if inputmode == "root":
        if inputstyle not in markerstyles_root2mpl:
            raise ValueError(
                "`{0}` is not a valid ROOT `markerstyle`".format(inputstyle)
            )
        if mode == "root":
            return inputstyle
        ...
        if inputstyle not in markerstyles_mpl2root:
            raise ValueError(
                "`{0}` is not a valid matplotlib `markerstyle`".format(inputstyle)
            )
        if mode == "mpl":
            return inputstyle
...
class MarkerStyle(_StyleContainer):
    ...
    __doc__ += "\n".join(["    '{0}'".format(x) for x in markerstyles_text2root])
...
def convert_linestyle(inputstyle, mode, inputmode=None):  # noqa: C901
    ...
    if inputmode is None:
        if inputstyle in linestyles_root2mpl:
            ...
            raise ValueError("`{0}` is not a valid `linestyle`".format(inputstyle))
    if inputmode == "root":
        if inputstyle not in linestyles_root2mpl:
            raise ValueError("`{0}` is not a valid ROOT `linestyle`".format(inputstyle))
        if mode == "root":
            return inputstyle
        ...
        if inputstyle not in linestyles_mpl2root:
            raise ValueError(
                "`{0}` is not a valid matplotlib `linestyle`".format(inputstyle)
            )
        if mode == "mpl":
            return inputstyle
    ...
    if inputmode is None:
        try:
            ...
            if inputstyle is None:
                ...
                raise ValueError("`{0}` is not a valid `fillstyle`".format(inputstyle))
    if inputmode == "root":
        if mode == "root":
            return inputstyle
        ...
        if inputstyle is not None and inputstyle[0] not in fillstyles_mpl2root:
            raise ValueError(
                "`{0}` is not a valid matplotlib `fillstyle`".format(inputstyle)
            )
        if mode == "mpl":
            return inputstyle
        if inputstyle is None:
...
class FillStyle(_StyleContainer):
    ...
    __doc__ = __doc__[: __doc__.rfind("\n") + 1]
    __doc__ += "\n".join(["    '{0}'".format(x) for x in fillstyles_text2root])
    if sys.version_info[0] < 3:
        del x
    __doc__ += """
