...
# New function for data, only rounding the error and not the value
def pdgRoundData(value, error):
    "Given a value and an error, round and format them according to the PDG rules for significant digits"
    ...
    tD = threeDigits(error)
    nD = nSignificantDigits(tD)
    _, expErr = frexp10(value)[1], frexp10(error)[1]
    extraRound = 1 if tD >= 950 else 0
    return formatValue(error, expErr, nD, extraRound)
